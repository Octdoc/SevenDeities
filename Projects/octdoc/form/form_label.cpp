...
namespace octdoc
{
	namespace form
	{
		Label::Label(const Window::P parent, const WCHAR text[], int x, int y, int w, int h)
			:Form(parent)
		{
			m_hwnd = CreateWindow(L"static", text, WS_VISIBLE | WS_CHILD, x, y, w, h, parent->getHWND(), NULL, GetModuleHandle(NULL), NULL);
		}
		Label::P Label::Create(const Window::P parent, const WCHAR* text)
		{
			return Create(parent, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, text);
		}
		Label::P Label::Create(const Window::P parent, int x, int y, const WCHAR* text)
		{
			return Create(parent, x, y, CW_USEDEFAULT, CW_USEDEFAULT, text);
		}
		Label::P Label::Create(const Window::P parent, int x, int y, int w, int h, const WCHAR* text)
		{
