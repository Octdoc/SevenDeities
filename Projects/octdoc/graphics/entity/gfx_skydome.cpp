...
namespace octdoc
{
	namespace gfx
	{
		...
		{
			ModelLoader ml;
			ml.CreateCube({ -1.0f, -1.0f, -1.0f }, { 2.0f, 2.0f, 2.0f }, (UINT)ShaderInputLayoutType::POSITION);
			ml.FlipInsideOut();
			m_model = Model::Create(device, ml);
			m_vertexShader = VertexShader::Create(device, L"Shaders/vsSky.cso", (UINT)ShaderInputLayoutType::POSITION);
